...
export const RealOperationsControls: React.FC<RealOperationsControlsProps> = ({
  ...
  const [gpsMode, setGpsMode] = useState<'LIVE' | 'OFFLINE'>('OFFLINE');
  ...
  const enableLiveGps = () => {
    ...
    if (!navigator.geolocation) {
      setGpsMode('OFFLINE');
      setGpsNotice('This browser cannot provide GPS. Exact device coordinates are unavailable.');
      return;
    }
    if (watchId.current !== null) navigator.geolocation.clearWatch(watchId.current);
    watchId.current = navigator.geolocation.watchPosition((position) => {
      ...
      setGpsNotice(error.code === error.PERMISSION_DENIED
        ? 'GPS permission was denied. Enable permission to share exact coordinates.'
        : 'Live GPS is unavailable. Exact device coordinates are unavailable.');
  ...
  const stopGps = () => {
    ...
    setGpsMode('OFFLINE');
    setGpsNotice('GPS stopped by command user.');
    onGpsLocation?.(null);
  };
    ...
    <section className="panel-card operations-panel" style={{ marginBottom: '1rem' }}>
      ...
      <div className="panel-body" style={{ display: 'grid', gap: '0.75rem' }}>
        <div className="ops-status-grid">
          ...
          <span className={`ops-status ${gpsMode === 'OFFLINE' ? 'offline' : 'real'}`}>GPS: {gpsMode}</span>
        ...
        <div className="gps-control-row">
          <div>
            ...
            {lastPosition && <small>REAL DEVICE • {lastPosition.latitude.toFixed(6)}, {lastPosition.longitude.toFixed(6)} • {new Date(lastPosition.timestamp).toLocaleTimeString()}</small>}
        ...
        <div className="ops-integrations-note">
          ...
          <span>AUTHENTICATED IN-APP WEBSOCKET ONLY</span>
        </div>
      </div>
    </section>
  );
