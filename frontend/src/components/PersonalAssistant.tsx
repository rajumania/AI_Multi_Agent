  ...
  return (
    <div className="personal-assistant-float">
      {open && (
        <section className="personal-assistant-panel" aria-label="AITAM Safety Assistant personal assistant" style={{ width: 'min(360px, calc(100vw - 2rem))', height: 480, display: 'flex', flexDirection: 'column', background: '#fff', border: '1px solid #c7d2fe', borderRadius: 16, boxShadow: '0 18px 45px rgba(15,23,42,.24)', overflow: 'hidden', marginBottom: '.7rem', animation: 'assistant-pop .18s ease-out' }}>
          <header className="personal-assistant-panel-header" style={{ display: 'flex', alignItems: 'center', gap: '.55rem', padding: '.75rem .85rem', color: '#fff', background: 'linear-gradient(135deg,#312e81,#0f766e)' }}>
            <Sparkles size={17} /><div style={{ flex: 1 }}><strong style={{ display: 'block', fontSize: '.82rem' }}>AITAM Safety Assistant</strong><span style={{ fontSize: '.65rem', color: '#c7d2fe' }}>Personal Assistant</span></div>
          ...
          <div className="personal-assistant-log" role="log" aria-live="polite" style={{ flex: 1, overflowY: 'auto', padding: '.8rem', background: '#f8fafc' }}>
            ...
            {!loadingHistory && messages.length === 0 && <div style={{ padding: '1rem .35rem', color: '#64748b', fontSize: '.76rem', lineHeight: 1.5 }}><Bot size={21} color="#6366f1" /><p style={{ marginTop: '.45rem' }}>Ask about community safety, reporting, or your available profile preferences.</p></div>}
      ...
      <form onSubmit={(event: FormEvent) => { event.preventDefault(); void send(); }} style={{ display: 'flex', gap: '.4rem', padding: '.65rem', borderTop: '1px solid #e2e8f0', background: '#fff' }}>
            <textarea aria-label="Message AITAM Safety Assistant" value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send(); } }} placeholder="Type a message…" rows={1} style={{ flex: 1, resize: 'none', padding: '.5rem .6rem', border: '1px solid #cbd5e1', borderRadius: 8, font: 'inherit', fontSize: '.76rem' }} />
      ...
      <button className="personal-assistant-float-button" aria-expanded={open} aria-label={open ? 'Close AITAM Safety Assistant' : 'Open AITAM Safety Assistant'} title={open ? 'Close AITAM Safety Assistant' : 'Open AITAM Safety Assistant'} onClick={() => setOpen((value) => !value)}><Sparkles size={17} /> <span className="sr-only">{open ? 'Close' : 'Open'} AITAM Safety Assistant</span>{unread && <span className="personal-assistant-unread" aria-label="Unread assistant response" />}{open ? <ChevronDown size={14} /> : null}</button>
    </div>
  );
};
